...
(function () {
  ...
  var T = {
    zh: {
      ...
      wall: '牌墙', turn: '轮到', dealer: '庄', shanten: '向听',
      ...
      stWon: '胡', stReady: '听牌', stNot: '未听', won: '胡', reveal: '亮牌',
      ...
      helpBody: '<b>渲染</b>：牌桌与牌由 <b>PixiJS（GPU）</b>绘制，按钮/弹层为 DOM（混合架构）。<br>' +
        ...
        '<b>底部教学条</b>：常驻显示「离听牌还差几张」；听牌时金框虚线牌就是你在等的牌，右下角数字是还剩几张。<br>' +
        '<b>★ 推荐</b>：金色光晕那张是建议打出的牌，教学条右侧会说明理由（打它之后进张多少张）。<br>' +
        '<b>难度</b>：右上角可切 新手 / 标准 / 困难 —— 只改对手的强弱，不改规则。<br>' +
        '<b>结算</b>：终局按「番数×底分」计分，另算杠分、查大叫、查花猪、退税，给出每家净分。<br>' +
        '<b>血战到底</b>：一家胡后本局继续，直到 3 家胡或牌墙摸完。',
      ...
      teachWinReason: function (t, f) { return '打 ' + t + ' → 听牌（' + f + '）'; },
      ...
      youWon: '你赢了',
      scoreNote: '分 = 番数 × 底分(1)；自摸由尚未胡牌者各付一份，点炮由放炮者独付；流局时未听牌者赔听牌者（查大叫），未打缺者赔花猪；未胡者收到的杠分退还。',
    ...
    en: {
      ...
      wall: 'Wall', turn: 'Turn', dealer: 'D', shanten: 'Shanten',
      ...
      helpBody: '<b>Rendering</b>: table and tiles are drawn with <b>PixiJS (GPU)</b>; buttons stay DOM.<br>' +
        ...
        '<b>Blood Battle</b>: play continues until 3 wins or the wall empties.',
  ...
  function newGame() {
    ...
    state.demo = false; state.overlay = null;
  ...
  async function buildTextures() {
    var TW = 132, TH = 185, n = 0, jobs = [];
    for (var id = 0; id < 27; id++) {
      jobs.push((function (i) { return svgToTexture(MJ.face(i), TW, TH).then(function (tx) { return [i, tx]; }); })(id));
    }
    jobs.push(svgToTexture(MJ.back(), TW, TH).then(function (tx) { return ['back', tx]; }));
  ...
  function tileNode(id, w, h, opts) {
    ...
    // 落地阴影（始终向下，统一光源）
    var sh = gfx();
    sh.roundRect(-fw / 2 + 2, -fh / 2 + depth + 3, fw, fh, rad).fill({ color: 0x000000, alpha: 0.26 });
    c.addChild(sh);
    // 厚度侧边
    var side = gfx();
    side.roundRect(-fw / 2, -fh / 2 + depth, fw, fh, rad).fill({ color: 0xd8cba9 });
    side.roundRect(-fw / 2, -fh / 2 + depth, fw, fh, rad).stroke({ width: 1, color: 0xa89874, alpha: 0.7 });
    c.addChild(side);
    // 牌面（旋转到朝桌心）
  ...
  function feltTexture(W, H) {
    ...
    var grd = x.createRadialGradient(cw / 2, ch * 0.86, Math.min(cw, ch) * 0.05, cw / 2, ch / 2, Math.max(cw, ch) * 0.78);
    grd.addColorStop(0, '#2a7a56');
    grd.addColorStop(0.4, '#1c6042');
    grd.addColorStop(0.72, '#134a32');
    grd.addColorStop(1, '#082416');
    x.fillStyle = grd; x.fillRect(0, 0, cw, ch);
    for (var i = 0; i < 2600; i++) {
      x.fillStyle = 'rgba(255,255,255,' + (Math.random() * 0.035).toFixed(3) + ')';
      x.fillRect(Math.random() * cw, Math.random() * ch, 1.4, 1.4);
    }
    for (var j = 0; j < 1600; j++) {
      x.fillStyle = 'rgba(0,0,0,' + (Math.random() * 0.05).toFixed(3) + ')';
  ...
  function drawFelt(W, H) {
    ...
    var rim = gfx();
    rim.roundRect(5, 5, W - 10, H - 10, 20).stroke({ width: 4, color: 0x061c12, alpha: 0.85 });
    rim.roundRect(9, 9, W - 18, H - 18, 17).stroke({ width: 2, color: 0xd9b45c, alpha: 0.45 });
    rim.roundRect(13, 13, W - 26, H - 26, 15).stroke({ width: 1, color: 0x6fae8c, alpha: 0.25 });
  ...
  function drawSeat(s, F, M) {
    ...
    if (!hide) {
      for (i = 0; i < hs.slots.length; i++) {
        ...
        node.position.set(sl.x, sl.y);
        node.__id = sl.id; node.__sep = sl.sep;
        if (isMe && !p.won) {
          node.eventMode = legal ? 'static' : 'none';
          node.cursor = legal ? 'pointer' : 'default';
          node.alpha = legal ? 1 : 0.42;
          (function (nd, y0) {
            nd.on('pointerover', function () { if (nd.alpha > 0.6) { nd.y = y0 - 12; nd.scale.set(1.06); } });
            nd.on('pointerout', function () { nd.y = y0; nd.scale.set(1); });
            nd.on('pointerdown', function () { if (nd.alpha > 0.6) humanDiscard(nd.__id); });
          })(node, sl.y);
  ...
  function computeTeach() {
    ...
    return {
      kind: 'live', sh: sh, shAfter: shAfter, myTurn: myTurn, mustMiss: mustMiss,
  ...
  function drawTeachStrip(W, H, M) {
    ...
    if (my.won) {
      ...
      // 有效向听：轮到我时用「打完推荐牌之后」的向听，更贴近真实进度
      var shEff = teach.myTurn && teach.rec >= 0 ? teach.shAfter : teach.sh;
      if (teach.waiting.length) {
        msg = (teach.myTurn && teach.rec >= 0 ? '打这张就听牌 · ' : '✅ ') +
  ...
  function render() {
    ...
    try {
      ...
      state.recTile = -1;
      if (state.g.phase === 'playing' && state.g.turn === HUMAN && !my.won && !state.g.claim) {
        try { state.recTile = CM.botDiscardTile(state.g, HUMAN, 'normal'); } catch (e2) { state.recTile = -1; }
      }
      state.teach = computeTeach();
  ...
  function renderChrome() {
    ...
    document.getElementById('hstat').innerHTML = G ? (
      ...
      '<span>' + t().shanten + ' <b>' + CM.handShanten(G.players[HUMAN]) + '</b></span>' +
  ...
  function drawActionsDOM(W, H) {
    var G = state.g, my = G.players[HUMAN], box = document.getElementById('actions');
    if (!box) {
  ...
  function resultModal() {
    ...
    for (var i = 0; i < order.length; i++) {
      ...
      var status = p.won ? t().stWon + ' ' + p.winInfo.fans.total + ' 番' : (G.readyAtEnd[s] ? t().stReady : t().stNot);
  ...
  function renderOverlay() {
    ...
    if (!G) { ov.className = 'overlay'; ov.innerHTML = ''; return; }
    ...
    var b;
    if ((b = document.getElementById('againBtn'))) b.onclick = newGame;
    if ((b = document.getElementById('closeRes'))) b.onclick = function () { state.overlay = 'none'; render(); };
    if ((b = document.getElementById('helpClose'))) b.onclick = function () { state.overlay = null; render(); };
  }
  function renderLog() {
  ...
  function fail(msg) {
    ...
    window.__PIXI_TABLE__ = { ready: false, error: String(msg) };
  }
  ...
  document.getElementById('newBtn').onclick = function () { if (state.ready) newGame(); };
