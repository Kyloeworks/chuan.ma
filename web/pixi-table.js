...
(function () {
  ...
  /* ================= 首屏资产加载进度 ================= */
  /**
   * 牌面纹理（28 张）与渲染器初始化都是异步的，加载期间得让用户看见「在动」。
   *
   * 两条真实感的来源，缺一不可：
   *   ① **真实台阶要够细** —— 纹理串行生成（见 buildTextures），每张一个台阶。
   *      并行时 28 张会在同一帧内全部完成，进度只能从 0 直跳 80%，看着就是假的。
   *   ② **显示值要比真实值平滑** —— 每帧用 requestAnimationFrame 向真实进度逼近，
   *      真实进度卡住时允许缓慢自爬（最多领先 5 个点），既不会僵在原地，也不会跑飞。
   * 另外脚本执行前的空窗由 CSS 预动画顶着（.bootbar i.creep），JS 接管时读它当前宽度接着走。
   */
  ...
  var bootUI = {
    el: null, bar: null, pct: null, step: null, ttl: null, err: null,
    target: 0, shown: 0, raf: 0, t0: 0, done: false, fading: false,
    marks: [],   // [{ratio,label}] 阶段断点 —— 文案按**显示值**反查，才能和百分比同步
  };
  var BOOT_MIN_MS = 600;    // 进度条最短停留时长（从 t0 起算），避免一闪而过
  var BOOT_CREEP = 0.05;    // 允许显示值领先真实进度的上限（5 个点）
  ...
  /** 把进度镜像到诊断对象（结构化出口，断言不依赖 DOM 文案） */
  function publishBoot() {
    window.__PIXI_TABLE__ = window.__PIXI_TABLE__ || {};
    window.__PIXI_TABLE__.boot = {
      ratio: +bootUI.shown.toFixed(4), pct: Math.round(bootUI.shown * 100), done: bootUI.done,
      label: bootUI.step ? bootUI.step.textContent : '',
      ms: Math.round(performance.now() - bootUI.t0),
    };
  }
  ...
  function bootReset() {
    ...
    bootUI.step.textContent = t().bootTtl;
    bootUI.marks = [{ ratio: 0, label: t().bootTtl }];
    // 接管 CSS 预动画：读它此刻的实际宽度（px），换算成比例接着走，避免进度条倒退回 0
    var w = parseFloat(getComputedStyle(bootUI.bar).width) || 0;
    var full = (bootUI.bar.parentNode && bootUI.bar.parentNode.clientWidth) || 1;
    bootUI.bar.classList.remove('creep');
    bootUI.shown = Math.max(0, Math.min(0.12, w / full));
    bootUI.bar.style.width = (bootUI.shown * 100).toFixed(1) + '%';
    publishBoot();
    if (!bootUI.raf) bootUI.raf = requestAnimationFrame(bootTick);
  }

  /** 推进真实进度（标签可选）；显示值由 bootTick 平滑跟随，文案也由它按显示值反查 */
  function bootPaint(ratio, label) {
    if (!bootEls()) return;
    bootUI.target = Math.max(bootUI.target, Math.max(0, Math.min(1, ratio)));
    if (label) {
      var lastMark = bootUI.marks[bootUI.marks.length - 1];
      if (!lastMark || lastMark.ratio !== bootUI.target || lastMark.label !== label) {
        bootUI.marks.push({ ratio: bootUI.target, label: label });
      }
    }
    if (!bootUI.raf && !bootUI.done) bootUI.raf = requestAnimationFrame(bootTick);
  }

  function bootTick() {
    bootUI.raf = 0;
    if (!bootEls()) return;
    var real = bootUI.target;
    // 目标值：至少是真实进度；真实进度停滞时允许领先 BOOT_CREEP，但**封顶 99%** ——
    // 100% 只由 bootFinish 宣布，否则会出现「条走满了但界面还没出来」。
    // ⚠️ 封顶只作用于「自爬」：一旦真实进度到了 1（bootFinish 已调用），目标就必须放开到 1，
    //    否则显示值永远停在 99%，下面的 s>=0.999 永远不成立 —— 进度条卡死，加载层不淡出。
    var cap = real >= 1 ? 1 : Math.min(real + BOOT_CREEP, 0.99);
    var goal = Math.min(cap, Math.max(real, bootUI.shown + 0.0012));
    var s = bootUI.shown + (goal - bootUI.shown) * 0.14;
    if (goal - s < 0.001) s = goal;
    bootUI.shown = s;
    bootUI.bar.style.width = (s * 100).toFixed(1) + '%';
    bootUI.pct.textContent = Math.round(s * 100) + '%';
    // 文案跟着**显示值**反查，而不是用「最新一次真实进度」的文案 ——
    // 否则会看到「初始化渲染器（GPU）· 17%」这种自相矛盾的组合：
    // 百分比还在平滑追纹理阶段的量，文案却已经跳到下一阶段了。
    if (bootUI.marks.length) {
      var lab = bootUI.marks[0].label;
      for (var mi = 0; mi < bootUI.marks.length; mi++) {
        if (bootUI.marks[mi].ratio <= s + 1e-6) lab = bootUI.marks[mi].label;
      }
      if (bootUI.step.textContent !== lab) bootUI.step.textContent = lab;
    }
    publishBoot();
    if (s >= 0.999) {          // 走满 → 标记完成并安排淡出
      bootUI.shown = 1;
      bootUI.bar.style.width = '100%';
      bootUI.pct.textContent = '100%';
      bootUI.step.textContent = t().bootReady;
      bootUI.done = true;
      publishBoot();
      bootFadeOut();
      return;
    }
    bootUI.raf = requestAnimationFrame(bootTick);
  }

  /** 加载完成：把目标推到 100%，进度条走满后停留到最短时长，再淡出加载层 */
  function bootFinish() {
    if (!bootEls()) return;
    bootUI.target = 1;
    if (!bootUI.raf) bootUI.raf = requestAnimationFrame(bootTick);
    // 兜底：rAF 在后台标签页、无头环境里会被节流甚至完全停跑，光靠逐帧逼近收不了尾。
    // 超时后无条件把进度落定到 100% 并安排淡出，保证「加载层一定会消失」。
    setTimeout(function () {
      if (bootUI.done) return;
      bootUI.shown = 1;
      bootUI.bar.style.width = '100%';
      bootUI.pct.textContent = '100%';
      bootUI.step.textContent = t().bootReady;
  ...
  /** 加载失败：进度条停住并写明原因，而不是让用户对着转不完的进度条 */
  function bootError(msg) {
    if (!bootEls()) return;
    if (bootUI.raf) { cancelAnimationFrame(bootUI.raf); bootUI.raf = 0; }
    bootUI.err.hidden = false;
    bootUI.err.textContent = t().bootFail + ' ' + msg;
    window.__PIXI_TABLE__ = window.__PIXI_TABLE__ || {};
    window.__PIXI_TABLE__.boot = {
      ratio: +bootUI.shown.toFixed(4), pct: Math.round(bootUI.shown * 100),
  ...
  /**
   * 逐张生成牌面纹理（27 张牌面 + 1 张牌背）。
   *
   * **刻意串行**：并行时 28 张会在同一帧里几乎同时完成，真实进度从起点直跳 80%，
   * 玩家看到的就是「前半程不动、最后一下满格」。串行后每张都是一级台阶，
   * 配合 bootTick 的逐帧逼近，进度条才是真的在走。onStep(done, total) 供首屏进度使用。
   */
  async function buildTextures(onStep) {
    var TW = 132, TH = 185, n = 0, done = 0, total = 28;
    function stepTex(id, tx) {
      if (tx) { state.tex[id] = tx; n++; }
      done++;
      if (onStep) onStep(done, total);
    }
    for (var id = 0; id < 27; id++) {
      stepTex(id, await svgToTexture(MJ.face(id), TW, TH));
    }
    stepTex('back', await svgToTexture(MJ.back(), TW, TH));
    state.texCount = n;
  }

  /* ================= Pixi 场景 ================= */
  /** onStep(ratio)：把「创建应用 → WebGL 上下文 → 挂场景」拆成可见的台阶 */
  async function initPixi(onStep) {
    if (onStep) onStep(0.84);
    var app = new PIXI.Application();
    if (onStep) onStep(0.86);
    await app.init({
      background: V.stage,
  ...
  function render() {
    ...
    try {
      // ── 尺寸与档位自洽（每帧一次，幂等） ──
      // PIXI 的 resizeTo 会在自己的 ticker 里调 renderer.resize，而 k 由 ResizeObserver 更新，
      // 两者不同步 —— 实测会出现「screen 已是 2560、k 还是 2.5(4K档)」的**稳定混排**：
      // 侧家手牌带按 4K 档算长度，比底部教学条还长 → 重叠 20px，而且不会自愈
      // （applyTierOnStageChange 有 key 去重，尺寸没再变就不会重绘，错误状态就留在屏上）。
      // 以舞台 DOM 尺寸为唯一真源，每帧把 screen 与 k 都对齐；k 只在跨档时变，不会来回抖。
      var stEl = document.getElementById('stage');
      var W = (stEl && stEl.clientWidth) || state.app.screen.width;
      var H = (stEl && stEl.clientHeight) || state.app.screen.height;
      if (state.app.screen.width !== W || state.app.screen.height !== H) {
        try { state.app.renderer.resize(W, H); } catch (e) { /* ignore */ }
      }
      var wantTier = TIERS[pickTierIdx(W, H)];
      if (wantTier.k !== state.k) {
        state.tier = wantTier.id;
        state.k = wantTier.k;
        try { document.documentElement.style.setProperty('--k', String(state.k)); } catch (e2) { /* ignore */ }
        requestRender();   // 新 --k 会改变顶栏高度，下一帧再排一次（跨档才发生，收敛）
      }
      var M = metrics(W, H);
      state.pulses = [];
      state.boxes = [];
  ...
  async function boot() {
    ...
    try {
      ...
      // 权重：牌面素材 4%→84%（每张一个台阶）· 渲染器 84%→93% · 首帧 93%→100%
      await buildTextures(function (d, n) { bootPaint(0.04 + 0.80 * d / n, t().bootTex(d, n)); });
      await initPixi(function (r) { bootPaint(r, t().bootGpu); });
      ...
      bootPaint(0.95, t().bootScene);
      applyTierOnStageChange();          // 按舞台尺寸定档（牌与字体同步缩放）
      openSetup();
      render();
      bootPaint(0.99, t().bootReady);
      bootFinish();                      // 资产齐了、首帧也画完了，把进度条走满再淡出
